...
from albumentations import (
    HorizontalFlip, VerticalFlip, ShiftScaleRotate, RandomBrightnessContrast, Transpose, Normalize, Compose, CLAHE, RandomRotate90, GaussianBlur, RandomCrop, CoarseDropout
)
...
def get_augmentations():
    return Compose([
        ...
        GaussianBlur(p=0.5),
        CoarseDropout(max_holes=8, max_height=8, max_width=8, p=0.5),  # Add Coarse Dropout for regularization
...
def create_model(num_classes):
    ...
    for layer in base_model.layers[:100]:  # Fine-tune only the top layers initially
